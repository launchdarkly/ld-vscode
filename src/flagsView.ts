  ...
  private flagToValues(flag: FeatureFlag): FlagValue {
    ...
    for(let i = 0; i<flagTargets.length; i++) {
        let curTarget = flagTargets[i]
          targets.push(new FlagValue(this.ctx, `Variation: ${flag.variations[curTarget.variation].name ? flag.variations[curTarget.variation].name : flag.variations[curTarget.variation].value}`, vscode.TreeItemCollapsibleState.None, [], "variation"),
    ...
    for(let i = 0; i<flag.variations.length; i++) {
      ...
      if (flag.variations[i].name) {
        variationValue = [new FlagValue(this.ctx, `${flag.variations[i].value}`, vscode.TreeItemCollapsibleState.None, [], 'value')]
      }

      variations.push(new FlagValue(this.ctx, `${flag.variations[i].name ? flag.variations[i].name : flag.variations[i].value}`, flag.variations[i].name ? vscode.TreeItemCollapsibleState.Collapsed : vscode.TreeItemCollapsibleState.None, variationValue, "name"))
    ...
    item.children.push(new FlagValue(this.ctx, `Variations`, vscode.TreeItemCollapsibleState.Collapsed, variations, 'variation'))

    item.children.push(new FlagValue(this.ctx, `Rule Count: ${flag.environments[this.config.env].rules.length}`, vscode.TreeItemCollapsibleState.None, [], "flagRules"))

    let fallThrough = flag.environments[this.config.env].fallthrough
    if (fallThrough.variation !== undefined) {
      item.children.push(new FlagValue(this.ctx, `Default Variation: ${flag.variations[fallThrough.variation].name ? flag.variations[fallThrough.variation].name : flag.variations[fallThrough.variation].value }`, vscode.TreeItemCollapsibleState.None, [], 'variationDefault'))
      ...
      for (let k = 0; k < fallThrough.rollout.variations.length; k++) {
        ...
        fallThroughRollout.push(
          ...
          new FlagValue(this.ctx, `Variation: ${flag.variations[fallThrough.rollout.variations[k].variation].name ?  flag.variations[fallThrough.rollout.variations[k].variation].name : flag.variations[fallThrough.rollout.variations[k].variation].value }`, vscode.TreeItemCollapsibleState.None, [], 'variation')
        )
      }
      item.children.push(new FlagValue(this.ctx, `Default Rollout`, vscode.TreeItemCollapsibleState.Collapsed, fallThroughRollout, 'rollout'))
    }

    if (flag.environments[this.config.env].offVariation !== undefined) {
      item.children.push(new FlagValue(this.ctx, `Off Variation: ${flag.variations[flag.environments[this.config.env].offVariation].name ? flag.variations[flag.environments[this.config.env].offVariation].name : flag.variations[flag.environments[this.config.env].offVariation].value}`, vscode.TreeItemCollapsibleState.None, [], 'variationOff'))
    }

    if (flag.defaults !== undefined) {
      item.children.push(new FlagValue(this.ctx, `Defaults`, vscode.TreeItemCollapsibleState.Collapsed, [
        new FlagValue(this.ctx, `OnVariation: ${flag.variations[flag.defaults.onVariation].name ? flag.variations[flag.defaults.onVariation].name : flag.variations[flag.defaults.onVariation].value }`, vscode.TreeItemCollapsibleState.None, [], 'variation'),
        new FlagValue(this.ctx, `OffVariation: ${flag.variations[flag.defaults.offVariation].name ? flag.variations[flag.defaults.offVariation].name : flag.variations[flag.defaults.offVariation].value}`, vscode.TreeItemCollapsibleState.None, [], 'variation')
        ])
      )
    }
    return item
