...
export default function toggleFlagCtxCmd(config: ILDExtensionConfiguration): Disposable {
	const toggleFlagCtxCmd = registerCommand(CMD_LD_TOGGLE_CTX, async (_, flagKey) => {
		try {
			const key = flagKey ? flagKey : (config.getCtx().workspaceState.get('LDFlagKey') as string);
			if (key) {
